var e={callbacks:{},init(e){this.callbacks=e},renderSidebar(e){e.innerHTML=`
    ...
    `,document.getElementById(`btn-tpl-generate`).onclick=()=>this.generateTemplate()},generateTemplate(){let e=document.getElementById(`tpl-name`).value.trim()||`MachineName`,t=document.getElementById(`tpl-ip`).value.trim()||`10.10.10.X`,n=`# Hack The Box - ${e}

## 📊 Summary
- **OS**: ${document.getElementById(`tpl-os`).value}
- **IP**: ${t}
- **Difficulty**: ${document.getElementById(`tpl-diff`).value}
...
### Nmap Scan
\`\`\`bash
nmap -p- --min-rate 1000 -T4 ${t}
nmap -p <PORTS> -sC -sV ${t} -oN nmap_initial.txt
\`\`\`
...
- 
- 
`;if(this.callbacks.createTab)this.callbacks.createTab(`${e}.md`,n);else{let e=this.callbacks.getEditor();e&&e.setValue(n)}}};export{e as TemplateGeneratorPanel};