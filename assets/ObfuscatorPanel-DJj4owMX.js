var e={callbacks:{},init(e){this.callbacks=e},renderSidebar(e){e.innerHTML=`
    ...
    `;let t=document.getElementById(`obf-raw-input`),n=document.getElementById(`obf-result`),r={url:e=>encodeURIComponent(e),"url-double":e=>encodeURIComponent(encodeURIComponent(e)),base64:e=>btoa(unescape(encodeURIComponent(e))),hex:e=>e.split(``).map(e=>`\\x`+e.charCodeAt(0).toString(16).padStart(2,`0`)).join(``),html:e=>e.split(``).map(e=>`&#`+e.charCodeAt(0)+`;`).join(``),concat:e=>e.split(``).map(e=>"'${c}'").join(` || `)};e.querySelectorAll(`.obf-btn`).forEach(e=>{e.onclick=()=>{let i=e.getAttribute(`data-type`),a=t.value;if(a)try{n.value=r[i](a)}catch(e){n.value=`Error encoding: `+e.message}}}),document.getElementById(`obf-copy`).onclick=()=>{n.select(),document.execCommand(`copy`),window.ToastManager&&window.ToastManager.show(`Copied to clipboard!`,`success`)}}};export{e as ObfuscatorPanel};